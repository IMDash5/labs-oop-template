...
CombatVisitor::CombatVisitor(std::shared_ptr<NPC> attacker, double attackRange, std::vector<std::shared_ptr<NPC>> &npcs, std::vector<std::shared_ptr<Observer>> &observers, std::vector<std::shared_ptr<NPC>> &toRemove)
    : attacker(attacker), attackRange(attackRange), npcs(npcs), observers(observers), toRemove(toRemove) {}

void CombatVisitor::notify(const std::string &event, std::vector<std::shared_ptr<Observer>> &observers)
{
    for (auto &observer : observers){observer->onEvent(event);}
}

void CombatVisitor::visit(std::shared_ptr<Orc> orc)
{
    if (attacker->distanceTo(orc) > attackRange){return;}
...
void CombatVisitor::visit(std::shared_ptr<Druid> druid){
    if (attacker->distanceTo(druid) > attackRange){return;}
...
void CombatVisitor::visit(std::shared_ptr<SlaveTrader> slaveTrader){
    if (attacker->distanceTo(slaveTrader) > attackRange){return;}
    if (attacker->getType() == "Orc"){
